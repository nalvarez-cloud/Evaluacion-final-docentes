...
try:
    ...
    PYSENTIMIENTO_IMPORT_ERROR = None
except Exception as e:
    create_analyzer = None
    PYSENTIMIENTO_IMPORT_ERROR = e
...
# RoBERTuito es obligatorio en esta versión.
# Si no carga, la app se detiene para mostrar el error real.
if PYSENTIMIENTO_IMPORT_ERROR is not None:
    st.error("RoBERTuito es obligatorio, pero pysentimiento no se pudo importar.")
    st.exception(PYSENTIMIENTO_IMPORT_ERROR)
    st.stop()
...
def cargar_modelos_propios():
    ...
    archivos_individuales = {
        ...
        "Modelo percepción docente": "modelo_percepcion_docente_svm_etiqueta_manual.pkl",
    }
...
def cargar_robertuito():
    # RoBERTuito es obligatorio. Si falla, el error se muestra en pantalla.
...
def limpiar_columnas_dataframe(df):
    ...
    columnas_validas = [
        ...
        if c.lower() not in ["", "unnamed: 0", "nan", "none"]
    ]
...
def leer_archivo_robusto(archivo):
    ...
    columnas_esperadas = {"Q Type", "Answer", "Answer Match", "# Responses"}
    ...
    for enc in encodings:
        for sep in separadores:
            try:
                ...
                candidatos.append((score, df_tmp))
            except Exception as e:
                errores.append((enc, sep, str(e)[:100]))

    if not candidatos:
        raise ValueError(f"No se pudo leer el archivo CSV. Errores: {errores[:5]}")
...
def analizar_sentimiento_robertuito(analyzer, texto):
    # RoBERTuito es obligatorio. No se devuelve "No disponible" silenciosamente.
...
try:
    ...
    st.error("RoBERTuito es obligatorio, pero no pudo cargarse correctamente.")
    st.exception(e)
    st.stop()
...
st.sidebar.write("**RoBERTuito:** disponible y obligatorio")
...
# RoBERTuito como benchmark externo obligatorio.
...
try:
    ...
    st.error("RoBERTuito cargó, pero falló al clasificar comentarios.")
    st.exception(e)
    st.stop()
